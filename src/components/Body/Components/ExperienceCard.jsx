...
export default function ProjectCard({ title, smdesc, techs, isLoaded, dated, datef }) {
    ...
    return (
        <li className={`max-w-full ${Hovered ? 'dark:bg-[#525252] bg-gray-50' : ''} px-2 md:max-w-1/2 lg:max-w-1/4 border-2 lg:border-0 relative origin-top rounded-lg ${isLoaded ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0'} transition-all duration-500 ease-in-out group lg:hover:!opacity-100 lg:group-hover/list:opacity-50`}>
            <div onMouseOver={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
                <div className='flex flex-col'>
                    ...
                    <div id="description_tags" className={`border-t-2 border-black dark:border-white py-2 flex flex-col lg:flex-row gap-3 justify-between origin-left ${isLoaded ? 'scale-x-100 opacity-100' : 'scale-x-0 opacity-0'} transition-all duration-300 ease-in-out delay-500`}>
                        <div className='my-auto'>
